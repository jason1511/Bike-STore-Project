...
namespace Bike_STore_Project
{
    internal sealed class WebsiteBikeEditorDialog : Form
    {
        ...
        public WebsiteBikeEditorDialog(WebsiteBike? bike=null)
        {
            ...
            BuildColors();LoadBrands();BuildLayout();if(_editing)Fill(Bike);UiTheme.Apply(this);
        }
        ...
        private void LoadBrands(){using var conn=Database.OpenConnection();using var cmd=conn.CreateCommand();cmd.CommandText="SELECT CAST(id AS TEXT),name FROM brands WHERE is_active=1 ORDER BY sort_order,name;";using var reader=cmd.ExecuteReader();while(reader.Read())_brand.Items.Add(new BrandChoice(reader.GetString(0),reader.GetString(1)));}
    ...
    internal sealed class WebsiteReceiveStockDialog : Form
    {
        ...
        public WebsiteReceiveStockDialog(IReadOnlyList<WebsiteBike> bikes,string? selectedId=null)
        {
        ...
        private void Save(){if(_bike.SelectedItem is not BikeChoice bike)return;var isNew=_color.SelectedItem is string;var name=isNew?_newColor.Text.Trim():((_color.SelectedItem as ColorChoice)?.Color.Name??"");if(string.IsNullOrWhiteSpace(name)){MessageBox.Show("Enter the new colour name.");return;}if(_cost.Value<=0){MessageBox.Show("Enter the unit purchase cost.");return;}BikeId=bike.Bike.Id;ColorName=name;ColorHex=_hex.Text;ColorImage=_image.Text.Trim();Quantity=(int)_quantity.Value;UnitCost=_cost.Value;ReceivedAt=_received.Value;Notes=_notes.Text.Trim();DialogResult=DialogResult.OK;Close();}
