...
namespace Bike_STore_Project
{
    internal sealed class WebsiteBikeEditorDialog : Form
    {
        ...
        private readonly CheckBox _featured=new(){Text=Strings.Get("Bike_Featured")},_active=new(){Text=Strings.Get("Inventory_Active"),Checked=true};
        ...
        {
            _editing=bike!=null;Bike=bike??new WebsiteBike();Text=_editing?Strings.Format("Bike_EditTitle",bike!.Brand,bike.Name):Strings.Get("Bike_AddTitle");
        ...
        {
            var tabs=new TabControl{Dock=DockStyle.Fill};var details=new TabPage(Strings.Get("Bike_DetailsTab"));var variants=new TabPage(Strings.Get("Bike_ColoursTab"));
            ...
            Add(table,Strings.Get("Bike_Id"),_id,0);Add(table,Strings.Get("Bike_BrandRequired"),_brand,0);Add(table,Strings.Get("Bike_ModelRequired"),_name,0);Add(table,Strings.Get("Inventory_SellingPrice"),_price,0);
            Add(table,Strings.Get("Bike_Battery"),_battery,2);Add(table,Strings.Get("Bike_Motor"),_motor,2);Add(table,Strings.Get("Bike_TopSpeed"),_speed,2);Add(table,Strings.Get("Bike_Range"),_range,2);Add(table,Strings.Get("Bike_MaxWeight"),_weight,2);Add(table,Strings.Get("Bike_Safety"),_safety,2);Add(table,Strings.Get("Bike_MainImage"),_image,2);Add(table,Strings.Get("Bike_Description"),_description,2);
            var flags=new FlowLayoutPanel{AutoSize=true};flags.Controls.Add(_featured);flags.Controls.Add(_active);Add(table,Strings.Get("Bike_Visibility"),flags,2);details.Controls.Add(table);
            ...
            variantRoot.Controls.Add(new Label{Text=Strings.Get("Bike_VariantHelp"),AutoSize=true,Tag="muted"},0,0);variantRoot.Controls.Add(_colors,0,1);
            var colorActions=new FlowLayoutPanel{Dock=DockStyle.Fill,AutoSize=true,WrapContents=true};var add=new Button{Text=Strings.Get("Bike_AddColour"),Width=120};var remove=new Button{Text=Strings.Get("Bike_RemoveSelected"),Width=135};add.Click+=(_,__)=>AddColor();remove.Click+=(_,__)=>RemoveColor();colorActions.Controls.Add(add);colorActions.Controls.Add(remove);variantRoot.Controls.Add(colorActions,0,2);variants.Controls.Add(variantRoot);
            tabs.TabPages.Add(details);tabs.TabPages.Add(variants);
            var actions=new FlowLayoutPanel{Dock=DockStyle.Bottom,Height=58,FlowDirection=FlowDirection.RightToLeft,Padding=new Padding(12)};var save=new Button{Text=Strings.Get("Bike_Save"),Width=130,Height=34};var cancel=new Button{Text=Strings.Get("Common_Cancel"),Width=90,Height=34,DialogResult=DialogResult.Cancel};save.Click+=(_,__)=>Save();actions.Controls.Add(save);actions.Controls.Add(cancel);Controls.Add(tabs);Controls.Add(actions);CancelButton=cancel;
        }

        private void BuildColors()
        {
            _colors.Columns.Add(new DataGridViewTextBoxColumn{Name="Name",HeaderText=Strings.Get("Bike_ColourName")});_colors.Columns.Add(new DataGridViewTextBoxColumn{Name="Hex",HeaderText=Strings.Get("Bike_HexCode")});
            _colors.Columns.Add(new DataGridViewTextBoxColumn{Name="Image",HeaderText=Strings.Get("Bike_ImagePath")});_colors.Columns.Add(new DataGridViewTextBoxColumn{Name="Stock",HeaderText=Strings.Get("Bike_StockQuantity"),ValueType=typeof(int)});
        }
    ...
    internal sealed class WebsiteReceiveStockDialog : Form
    {
        ...
        public WebsiteReceiveStockDialog(IReadOnlyList<WebsiteBike> bikes,string? selectedId=null,bool usesPurchaseCost=true)
        {
            ...
            Text=Strings.Get("Inventory_Receive");StartPosition=FormStartPosition.CenterParent;ClientSize=new Size(610,550);MinimumSize=new Size(520,460);FormBorderStyle=FormBorderStyle.Sizable;MinimizeBox=false;AutoScaleMode=AutoScaleMode.Dpi;
        ...
        private void Build(){var table=new TableLayoutPanel{Dock=DockStyle.Fill,ColumnCount=2,Padding=new Padding(24),AutoScroll=true};table.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,175));table.ColumnStyles.Add(new ColumnStyle(SizeType.Percent,100));Add(table,Strings.Get("Stock_Bicycle"),_bike);Add(table,Strings.Get("Stock_ExistingNewColour"),_color);Add(table,Strings.Get("Stock_NewColourName"),_newColor);Add(table,Strings.Get("Stock_ColourHex"),_hex);Add(table,Strings.Get("Stock_ColourImage"),_image);Add(table,Strings.Get("Stock_QuantityReceived"),_quantity);if(_usesPurchaseCost)Add(table,Strings.Get("Stock_UnitCost"),_cost);else Add(table,Strings.Get("Stock_Online"),new Label{Text=Strings.Get("Stock_OnlineHelp"),AutoSize=true,Tag="muted"});Add(table,Strings.Get("Stock_ReceivedAt"),_received);Add(table,Strings.Get("Stock_Notes"),_notes);Add(table,Strings.Get("Stock_After"),_summary);var actions=new FlowLayoutPanel{AutoSize=true,FlowDirection=FlowDirection.RightToLeft};var save=new Button{Text=Strings.Get("Inventory_Receive"),Width=125,Height=34};var cancel=new Button{Text=Strings.Get("Common_Cancel"),Width=90,Height=34,DialogResult=DialogResult.Cancel};save.Click+=(_,__)=>Save();actions.Controls.Add(save);actions.Controls.Add(cancel);Add(table,"",actions);Controls.Add(table);CancelButton=cancel;}
        private void LoadColors(){_color.Items.Clear();if(_bike.SelectedItem is BikeChoice choice){foreach(var c in choice.Bike.Colors)_color.Items.Add(new ColorChoice(c));}_color.Items.Add(Strings.Get("Stock_AddNewColour"));_color.SelectedIndex=0;UpdateNewColourFields();}
        ...
        private void UpdateSummary(){var current=_color.SelectedItem is ColorChoice c?c.Color.StockQty:0;_summary.Text=Strings.Format("Stock_Summary",current,(int)_quantity.Value,current+(int)_quantity.Value);}
        private void Save(){if(_bike.SelectedItem is not BikeChoice bike)return;var isNew=_color.SelectedItem is string;var name=isNew?_newColor.Text.Trim():((_color.SelectedItem as ColorChoice)?.Color.Name??"");if(string.IsNullOrWhiteSpace(name)){MessageBox.Show(Strings.Get("Stock_EnterColour"));return;}if(_usesPurchaseCost&&_cost.Value<=0){MessageBox.Show(Strings.Get("Stock_EnterCost"));return;}BikeId=bike.Bike.Id;ColorName=name;ColorHex=_hex.Text;ColorImage=_image.Text.Trim();Quantity=(int)_quantity.Value;UnitCost=_cost.Value;ReceivedAt=_received.Value;Notes=_notes.Text.Trim();DialogResult=DialogResult.OK;Close();}
        ...
        private sealed record ColorChoice(WebsiteBikeColor Color){public override string ToString()=>Strings.Format("Stock_Choice",Color.Name,Color.StockQty);}
    }
}
