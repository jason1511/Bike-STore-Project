...
FROM audit_log ORDER BY datetime(created_at) DESC,id DESC;");
        ...
        private void GenerateReport()
        {
            ...
            decimal stockIn = Scalar(conn, @"SELECT COALESCE(SUM(CASE WHEN quantity_change>0 THEN quantity_change ELSE 0 END),0) FROM stock_movements WHERE movement_type='STOCK_IN' AND datetime(created_at)>=datetime($from) AND datetime(created_at)<datetime($to);", from, to);
